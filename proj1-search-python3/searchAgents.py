...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.food_eaten = [False for i in range(4)]
    ...
    def getStartState(self):
        ...
        space)
        """
        return (self.startingPosition, tuple(self.food_eaten))
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                successors.append(
                    (((nextx, nexty), tuple(food_eaten)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    goals = []
    curr_pos, food_eaten = state
    for i in range(4):
        if not food_eaten[i]:
            goals.append(corners[i])
    # print(goals)

    distances = [manhattanHeuristic(
        curr_pos, Subproblem(goal)) for goal in goals]
...
def foodHeuristic(state, problem):
    ...
    distances = [mazeDistance(
        curr_pos, goal,problem.startingGameState) for goal in goals]
